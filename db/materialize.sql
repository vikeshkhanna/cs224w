...
INSERT OR IGNORE INTO fork SELECT * FROM (SELECT A.actor, B.actor, max(A.created_at, B.created_at) created_at, 0 from fork A, fork B where A.repo_name = B.repo_name and A.actor < B.actor union SELECt actor, owner, fork.created_at, 1 from fork, repository where fork.repo_name = repository.name);
